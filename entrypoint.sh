...
# Vorsicht, ssl-mode=DISABLED ist nur im bestehenden Docker-Setup praktikabel wg. TLS/SSL error: self-signed certificate und ist eher eine Notlösung
mysql --ssl-mode=DISABLED -h "$HOST" -u "$USER" -p"$PASS" "$DB" < /usr/src/app/db_scheme.sql
